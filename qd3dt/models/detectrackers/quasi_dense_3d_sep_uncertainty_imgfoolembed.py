...
class QuasiDense3DSepUncertaintyImgFoolEmbed(BaseDetector, RPNTestMixin, BBoxTestMixin,
                                 Analyze3DMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(QuasiDense3DSepUncertaintyImgFoolEmbed, self).__init__()
        ...
        #####################################################################
        self.embed_backbone = builder.build_backbone(backbone)
        self.embed_backbone.init_weights(pretrained='http://dl.yf.io/dla/models/imagenet/dla34-ba72cf86.pth')

        embed_neck = {'type': 'DLAUp', 'channels': [256, 256, 256, 256], 'scales': (1, 2, 4, 8), 'in_channels': [64, 128, 256, 512], 'num_outs': 5}
        self.embed_neck = builder.build_neck(embed_neck)
        self.embed_neck.init_weights()
        #####################################################################    
    ...
    def init_weights(self, pretrained=None):
        super(QuasiDense3DSepUncertaintyImgFoolEmbed, self).init_weights(pretrained)
    ...
    def embed_extract_feat(self, img):
        x = self.embed_backbone(img)
        x = self.embed_neck(x)
        return x
    ...
    def forward_train(self,
        ...
        embed_x = self.embed_extract_feat(img)
        embed_ref_x = self.embed_extract_feat(ref_img)
        ...
        key_pos_feats = self.embed_roi_extractor(embed_x[:self.espool], key_pos_rois)
        key_embeds, key_depth = self.embed_head(key_pos_feats)

        # ref gt embeds
        ref_gt_rois = bbox2roi(ref_gt_bboxes)
        ref_gt_feats = self.embed_roi_extractor(embed_ref_x[:self.espool],
                                                ref_gt_rois)
        ...
        if self.train_cfg.embed.with_ref_pos:
            ...
            ref_pos_feats = self.embed_roi_extractor(embed_ref_x[:self.espool],
                                                     ref_pos_rois)
        ...
        if self.train_cfg.embed.with_ref_neg:
            ...
            ref_neg_feats = self.embed_roi_extractor(embed_ref_x[:self.espool],
                                                     ref_neg_rois)
        ...
        if self.train_cfg.embed.with_key_neg:
            ...
            key_neg_feats = self.embed_roi_extractor(embed_x[:self.espool],
                                                     key_neg_rois)
    ...
    def simple_test(self,
        ...
        if det_bboxes.size(0) != 0:
            ...
            embed_feats = self.embed_roi_extractor(embed_x[:self.spool], embed_rois)
